...
ADAPTER_PATH    = "./output_peft/mistral-7b/lora_adapter"
TEST_DATA_PATH  = "test_data.jsonl"
OUTPUT_DIR      = "./eval_results/mistral-7b"
...
def score_tc1(raw_output, expected):
    ...
    pred_cv = predicted.get("control_variables", [])
    pred_controls = set(v if isinstance(v, str) else str(v) for v in pred_cv)
    exp_cv = expected.get("control_variables", [])
    exp_controls = set(v if isinstance(v, str) else str(v) for v in exp_cv)
